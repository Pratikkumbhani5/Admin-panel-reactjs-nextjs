...
const UserList = () => {
  ...
  const columns = [
...

    {
      ...
      sortable: true,
      // width: "210px",
      cell: (row) => <div>{capitalizeFirstLetter(row.first_name)}</div>,
    },
    {
      name: "Last Name",
      selector: last_name,
      sortable: true,
      // width: "210px",
      cell: (row) => <div>{capitalizeFirstLetter(row.last_name)}</div>,
    },
    {
      name: "Email",
      selector: email,
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          {row.role == 1 && "Super Admin (1)"}
          {row.role == 2 && "Manager (2)"}
          {row.role == 3 && "Content (3)"}
          {row.role == 4 && "Sales (4)"}
        </div>
      ),
    },
    {
      ...
      cell: (row) => (
        <div>
          <Link href={`/admin/edit-user/${row.id}`}>
            <button
              type="button"
  ...
  const getUserData = async () => {
    await axios
      .get(`${process.env.NEXT_PUBLIC_APP_Base_URLa}/author/users`, {
        headers: {
  ...
  const deleteUser = async (id) => {
    if (session.status === "authenticated") {
      ...
      await axios
        .delete(
          `${process.env.NEXT_PUBLIC_APP_Base_URLa}/author/user-delete/${id}`,
          {
            headers: {
              Authorization: `Bearer ${session.data.user.token}`,
            },
